    ...
    fileFilter: (req, file, cb) => {
        ...
        if (file.fieldname === "st_image") {
            if (!allowedImageTypes.includes(file.mimetype)) {
                return cb(new Error("Solo se admiten archivos JPG, PNG, WEBP")); // "รองรับเฉพาะไฟล์ JPG, PNG, WEBP"
            }
        } else if (file.fieldname.startsWith("doc_")) {
            if (!allowedDocTypes.includes(file.mimetype)) {
                return cb(new Error("Solo se admiten archivos PDF, JPG, PNG")); // "รองรับเฉพาะไฟล์ PDF, JPG, PNG"
            }
        }
        cb(null, true);
    },
