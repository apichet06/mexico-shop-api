...
export const listBylgCode = asyncHandler(async (_req, res) => {
    ...
    // Si no se especifica el catalog, usa la única tienda principal; el query anterior se mantiene por compatibilidad con datos históricos (ถ้าไม่ได้ระบุ catalog ให้ใช้ร้านหลักเพียงร้านเดียว ส่วน query เดิมยังรองรับไว้สำหรับข้อมูลย้อนหลัง)
...
export const update = asyncHandler(async (req, res) => {
    ...
    await category.updateCategory(Number(cl_id), { cl_name });
