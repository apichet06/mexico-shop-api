...
import { ensureLocationMexicoSchema } from "./location.schema.js";
import { getSubWarehouseName, MAIN_WAREHOUSE_NAME } from "./location-name.js";
...
async function normalizeLocationNames(conn: Awaited<ReturnType<typeof pool.getConnection>>, stId: number): Promise<void> {
    ...
    for (const row of rows) {
        ...
        await conn.query(
            ...
            [isDefault ? 1 : 0, isDefault ? MAIN_WAREHOUSE_NAME : getSubWarehouseName(subCount++), row.loc_id],
        );
    }
}

export async function ListLocations(): Promise<LocationsDTO[]> {
    await ensureLocationMexicoSchema();
...
export async function getLocationById(st_id: number): Promise<LocationsDTO[]> {
    ...
    const [rows] = await pool.query<(RowDataPacket & LocationsDTO)[]>(
        `SELECT a.*, b.st_company_name, a.state as province_name,
            COALESCE(a.municipality, a.city) as district_name, a.colonia as subdistrict_name
        FROM Locations a 
...
export async function CreateLocation(input: CreateLocationInput): Promise<number> {
    ...
    try {
        ...
        if (isDefault) {
            ...
            for (const [index, row] of oldDefaultRows.entries()) {
                await conn.query(
                    ...
                    [getSubWarehouseName(subWarehouseCount + index + 1), row.loc_id],
                );
            }
        ...
        if (isDefault) {
            ...
            loc_name = getSubWarehouseName(count + 1);
        }
    ...
    try {
        ...
        if (nextIsDefault) {
            ...
            if (oldDefaultRows.length > 0) {
                await conn.query(
                    "UPDATE Locations SET loc_name = ? WHERE loc_id = ?",
                    [getSubWarehouseName(count + 1), oldDefaultRows[0]!.loc_id]
                );
            }

            updateData.loc_name = MAIN_WAREHOUSE_NAME;
            ...
            updateData.loc_name = getSubWarehouseName(count + 1);
            ...
            if (current.is_default === 1) {
                ...
                if (nextDefault) {
                    await conn.query(
                        ...
                        [MAIN_WAREHOUSE_NAME, nextDefault.loc_id],
                    );
                }
            }
        }
