...
// แจ้งเตือน platform store (เจ้าของเว็บไซต์) ทาง notification bell เมื่อคืนเงินผ่าน Omise ไม่สำเร็จ ต้องโอนคืนลูกค้าเอง
...
function pickShippingOption(options: CalculateResult[], shippingScId?: number | null, storeName?: string): CalculateResult {
    ...
    if (!availableOptions.length) {
        throw new ApiError(400, storeName ? `ร้าน "${storeName}" ยังไม่มีอัตราค่าส่งที่ใช้ได้สำหรับที่อยู่นี้` : "ยังไม่มีอัตราค่าส่งที่ใช้ได้สำหรับที่อยู่นี้");
    }
    ...
    if (shippingScId && !selected) {
        throw new ApiError(400, storeName ? `ขนส่งที่เลือกสำหรับร้าน "${storeName}" ยังไม่พร้อมใช้งานสำหรับที่อยู่นี้` : "ขนส่งที่เลือกยังไม่พร้อมใช้งานสำหรับที่อยู่นี้");
    }
...
export async function requestRefund(or_id: number, u_id: number, reason: string, lg_code = "es", returnTracking = "", imageFiles: Express.Multer.File[] = [], selectedItems: RefundItemSelection[] = []): Promise<OrderDetailDTO> {
    ...
    try {
        ...
        if (itemRows.length === 0) throw new ApiError(404, "ไม่พบรายการสินค้าในคำสั่งซื้อนี้");
        ...
        if (statusCode === "DELIVERED") {
            if (selectedItems.length === 0) {
                throw new ApiError(400, "กรุณาเลือกรายการสินค้าที่ต้องการคืน");
            }
            ...
            for (const sel of selectedItems) {
                ...
                if (!Number.isInteger(qty) || qty <= 0) {
                    throw new ApiError(400, "จำนวนสินค้าที่เลือกคืนไม่ถูกต้อง");
                }
            ...
            includedItems = [...qtyByOiId.entries()].map(([oiId, qty]) => {
                ...
                if (!item) throw new ApiError(400, "รายการสินค้าที่เลือกไม่ถูกต้อง");

                const alreadyProtected = protectedQtyMap.get(oiId) ?? 0;
                const remaining = item.qty - alreadyProtected;
                if (qty > remaining) {
                    throw new ApiError(409, "จำนวนที่เลือกคืนเกินกว่าจำนวนที่เหลืออยู่ (บางส่วนอาจถูกคืนไปแล้ว)");
                }
...
export async function approveRefundRequest(or_id: number, st_id: number, note = "", lg_code = "es"): Promise<AdminOrderDetailDTO> {
    ...
    try {
        ...
        await conn.query(
            "UPDATE Refunds SET status = 'succeeded', refund_method = 'omise', remark = ?, updated_at = ? WHERE refund_id = ?",
...
export async function adminCancelOrder(or_id: number, st_id: number, note = "", lg_code = "es"): Promise<AdminOrderDetailDTO> {
    ...
    try {
        ...
        if (!ADMIN_CANCELLABLE_STATUS_CODES.includes(order.status_code as OrderStatusCode)) {
            throw new ApiError(400, "ยกเลิกคำสั่งซื้อได้เฉพาะที่ยังรอชำระเงินหรือชำระเงินแล้วเท่านั้น");
        }
        if (order.refund_status === "pending") {
            throw new ApiError(400, "คำสั่งซื้อนี้มีคำขอคืนเงินรอตรวจสอบอยู่แล้ว กรุณาดำเนินการคำขอคืนเงินก่อน");
        }
        ...
        const paymentRef = paymentRows[0]?.payment_ref ?? null;
        if (!paymentRef) throw new ApiError(400, "ไม่พบ reference การชำระเงินสำหรับคืนเงิน");

        const [refundRes] = await conn.query<ResultSetHeader>(
            `INSERT INTO Refunds (or_id, payment_ref, amount, status, remark, created_at, updated_at)
             VALUES (?, ?, ?, 'pending', ?, ?, ?)`,
            [or_id, paymentRef, Number(order.grand_total).toFixed(2), trimmedNote || "แอดมินยกเลิกคำสั่งซื้อ", new Date(), new Date()]
        );
        ...
        if (refundSucceeded) {
            const remark = [trimmedNote, "คืนเงินผ่าน Omise สำเร็จ"].filter(Boolean).join(" | ");
            await conn.query(
                "UPDATE Refunds SET status = 'succeeded', refund_method = 'omise', remark = ?, updated_at = ? WHERE refund_id = ?",
    ...
    try {
        ...
        if (refund.status_code === "RETURN_REQUESTED") {
            await setOrdersStatus(conn, [or_id], "DELIVERED", {
                remark: `คำขอคืนสินค้าไม่ผ่านการอนุมัติ: ${note.trim()}`,
            });
        }
...
export async function confirmManualRefundRequest(or_id: number, st_id: number, note = "", lg_code = "es"): Promise<AdminOrderDetailDTO> {
    ...
    try {
        ...
        // ร้านที่ไม่ใช่ platform เองไม่ได้ถือเงินลูกค้าจริง (platform เป็น merchant of record ผ่าน Omise)
        ...
        if (!(await isPlatformStore(refund.order_st_id)) && !(await isPlatformStore(st_id))) {
            throw new ApiError(403, "เฉพาะผู้ดูแลระบบ platform เท่านั้นที่ยืนยันการโอนคืนเงินให้ร้านนี้ได้");
        }
